document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 0. DEFINICIÓN DE ICONOS SVG (Accesibles)
    // ==========================================
    const SVGS = {
        sun: `<svg aria-hidden="true" focusable="false" fill="none" viewBox="0 0 24 24" stroke="orange" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>`,
        cloud: `<svg aria-hidden="true" focusable="false" fill="none" viewBox="0 0 24 24" stroke="#64748b" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"></path></svg>`,
        rain: `<svg aria-hidden="true" focusable="false" fill="none" viewBox="0 0 24 24" stroke="#3b82f6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><line x1="16" y1="13" x2="16" y2="21"></line><line x1="8" y1="13" x2="8" y2="21"></line><line x1="12" y1="15" x2="12" y2="23"></line><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"></path></svg>`,
        snow: `<svg aria-hidden="true" focusable="false" fill="none" viewBox="0 0 24 24" stroke="#0ea5e9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><line x1="8" y1="8" x2="16" y2="16"></line><line x1="8" y1="16" x2="16" y2="8"></line><line x1="12" y1="2" x2="12" y2="22"></line><line x1="2" y1="12" x2="22" y2="12"></line></svg>`,
        bolt: `<svg aria-hidden="true" focusable="false" fill="none" viewBox="0 0 24 24" stroke="#eab308" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>`,
        ...
        monte: `<svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%; height:100%;"><path d="m8 3 4 8 5-5 5 15H2L8 3z" /></svg>`,
        mapPin: `<svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:100%; height:100%;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>`,
        heart: `<svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" fill="currentColor" stroke="none" style="width:100%;height:100%;"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>`
    };
    ...
    window.renderMap = function () {
        ...
        if (filteredSpots.length === 0) {
            listContainer.innerHTML = '<div style="text-align:center; margin-top:30px; color:#475569;"><p>No se encontraron spots con estos filtros.</p></div>';
            return;
        }
        ...
        filteredSpots.forEach(spot => {
            ...
            const customIcon = L.divIcon({
                ...
                html: `
                    <div class="custom-marker ${mapPinClass}" 
                         role="button" 
                         tabindex="0"
                         aria-label="Ver detalles del spot: ${spot.nombre} en el mapa"
            ...
            item.setAttribute('aria-label', `Mostrar detalles de ${spot.nombre}, en ${spot.municipio.nombre}`);
            ...
            if (spot.etiquetas && spot.etiquetas.length > 0) {
                ...
                tagsHtml = `<div style="display:flex; gap:4px; margin-top:4px;" aria-hidden="true">
                    ${tagsToShow.map(t => `<span style="font-size:0.65rem; background:#f1f5f9; padding:2px 6px; border-radius:4px; color:#475569;">${t.nombre}</span>`).join('')}
                    ${spot.etiquetas.length > 2 ? '<span style="font-size:0.65rem; color:#64748b;">+</span>' : ''}
                 </div>`;
            }

            // CORRECCIÓN: Botón favorito usando class visually-hidden
            item.innerHTML = `
                ...
                <button class="btn-fav-action ${heartClass}" 
                        aria-pressed="${isFav}" 
                        onclick="toggleFav(${spot.id}, this)" 
                        data-spot-id="${spot.id}">
                    <span class="visually-hidden">${isFav ? 'Eliminar' : 'Añadir'} ${spot.nombre} de favoritos</span>
                    <div style="width: 18px; height: 18px;" aria-hidden="true">${SVGS.heart}</div>
                </button>
            `;
    ...
    window.openSpotDrawer = function (spot) {
        ...
        let tagsDrawerHtml = '';
        if (spot.etiquetas && spot.etiquetas.length > 0) {
            tagsDrawerHtml = `<div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:15px;" aria-label="Etiquetas">
        ...
        // CORRECCIÓN: Botón de cerrar y botón de favoritos con span.visually-hidden
        drawer.innerHTML = `
            <div class="drawer-header" style="background-image: url('${imageUrl}');">
                <button class="btn-close-drawer" onclick="closeSpotDrawer()">
                    <span class="visually-hidden">Cerrar detalles del spot</span>
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="drawer-body">
                <h2 class="drawer-title" tabindex="-1">${spot.nombre}</h2>
                <div class="drawer-subtitle">
                    <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                    ${spot.municipio.nombre} • ${typeLabel}
                </div>

                ${tagsDrawerHtml}

                <div id="weather-widget-${spot.id}" class="weather-widget" aria-label="Información del tiempo actual">
                ...
                <div class="drawer-actions">
                    <button class="btn-drawer-fav ${favClass}" 
                            aria-pressed="${isFav}" 
                            onclick="toggleFav(${spot.id}, this)" 
                            data-spot-id="${spot.id}">
                        <span class="visually-hidden">${isFav ? 'Eliminar de favoritos' : 'Añadir a favoritos'}</span>
                        <div style="width: 24px; height: 24px;" aria-hidden="true">${SVGS.heart}</div>
                    </button>

                    <a href="/spots/${spot.id}" class="btn-details-full" aria-label="Ver ficha completa de ${spot.nombre}">
                        Ver Ficha
                    </a>
                      
                    <a href="${googleMapsUrl}" target="_blank" rel="noopener noreferrer" class="btn-directions-map" aria-label="Abrir ${spot.nombre} en Google Maps">
                        <div style="width: 20px; height: 20px;" aria-hidden="true">${SVGS.mapPin}</div>
                        Mapa
                    </a>
                </div>
            </div>
    ...
    async function fetchWeather(lat, lng, elementId) {
        try {
            ...
            if (widget) {
                widget.innerHTML = `
                    <div style="width: 50px; height: 50px; flex-shrink: 0;" aria-hidden="true">${iconSvg}</div>
                    <div>
    ...
    window.toggleFav = function (spotId, btn) {
        ...
        fetch(url, {
            ...
            // CORRECCIÓN: Actualizamos el texto oculto del span en vez del aria-label
            allButtons.forEach(button => {
                ...
                if (data.status === 'added') {
                    ...
                    if(hiddenSpan) {
                        hiddenSpan.textContent = hiddenSpan.textContent.replace('Añadir', 'Eliminar');
                    }
                } else {
                    ...
                    if(hiddenSpan) {
                        hiddenSpan.textContent = hiddenSpan.textContent.replace('Eliminar', 'Añadir');
                    }
                }
            });
